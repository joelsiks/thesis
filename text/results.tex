...
If aligning allocation requests to 8 bytes for all allocators, the worst case is to allocate the smallest possible size and add one byte, i.e 17 bytes. Allocating 17-bytes means padding the allocation request to 24 bytes, essentially wasting 7 bytes of memory, whilst making the block header associated with the request take up a large amount of space relative to the request.

In the worst case, the reference version will waste up to 57.5\%, general version 69.6\% and the optimized version 29.2\%, if filling the heap with as many allocations as possible of 17 bytes.

Looking at the best case instead, where there is no padding necessary for alignment, the reference version will waste up to 50\%, general version 66\% and the optimized version 0\%, if filling the heap with as many allocations as possible of 16 bytes.
