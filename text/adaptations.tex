...
% Ett use-case där vi inte kommer att använda free-anrop eller att vi använder free-anrop men vill inte coalesce:a block. DeferredCoalescing.
% Bygger vidare på att konstruera allokatorn från livemapen där vi kan jobba i en cykel som består av clear -> free_ranges -> allokera -> repeat
% eller i fall där vi inte vill ta kostnaden att coalesce:a block direkt utan väljer att avvakta med det och göra en full-sweep senare.
% Detta tillåtter oss att inte behöva spara prev_phys_block pekaren i block headern, vilket gör att overheaden blir mindre.
