...
\begin{figure}[H]
    \centering
    \includesvg[width=0.9\textwidth]{figures/bitmap_flattening.svg}
    \vspace*{4mm}
...
The correlation between the new bitmap and free-lists is depicted in Figure~\ref{fig:bitmap_relationship}, adhering closely to the original TLSF design. The most granularity in size-ranges for the free-lists are found for the smallest allocation sizes, which aligns well with most allocations being small inside most Java programs. This means that less memory will potentially be wasted and less splitting done since blocks match the request size more often, thus saving performance.

\begin{figure}[H]
    \centering
    \vspace*{0.2cm}
    \includesvg[width=1\textwidth]{figures/bitmap_relationship.svg}
    \vspace*{1mm}
