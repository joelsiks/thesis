...
This section covers the implementation details that are relevant for the adaptations described in the previous section.

\subsection{Allocator Configurations}

The reference implementation of the allocator has been abstracted into a base class named \texttt{TLSFBase}. From the base class, the user must provide a set of configuration variables that define how the allocator behaves, where each unique set of configuration variables define a new allocator. An allocator may override the implementation of the methods defined in the base class so that computations may be done differently. For example, this allows different versions to apply different methods for allocation, freeing and support for concurrency. The configuration variables are:

\begin{enumerate}
    \item Number of next- and first-levels (Sets the size range granularity and number of free-list)
...
Implementing concurrency for the allocator will require being able to concurrently operate on its free-lists. The free-lists, in combination with the bitmaps that store data about them, are the critical sections of the allocator. In the reference design, the free-lists are designed as doubly-linked lists. However, lock-free operations on a doubly-linked-list will not be considered due to the complex operations required to support it. Instead, the appearance of the free-lists are simplified to make it easier to support concurrent operations.
...
M. Herlihy and N. Shavit~\cite[Chapter 11]{artofmpprogramming} describes the design and inner-workings of a lock-free stack from which the concurrent implementation of the allocator is largely based on. For reference, a general overview of how a lock-free stack work and how it connects to the free-lists inside the allocator is described.
...
Version tagging is described by D. Dechev et al.~\cite{bjarne_aba} as a common and effective solution for the ABA problem. However, due to how compare-and-exchange work, it is desirable to store the pointer and version tag in a single word, i.e. 8-bytes on a 64-bit system. 
